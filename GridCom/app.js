document.querySelector('#paint').addEventListener('click', (element) => {
    element.preventDefault();
    document.querySelector("#headerTable").addEventListener("submit", element => {
        element.preventDefault();
    });
    const dataArr = [];
    const data = document.querySelectorAll('tbody tr td:last-child');
    console.log(data);
    Array.from(data).forEach((el, index) => {
        dataArr.push([`var${index}`, +el.innerHTML.trim()]);
    })
    ...
    d3.csv("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/data_doubleHist.csv", function (data) {
        ...
        svg.append("text").attr("x", 320).attr("y", 30).text("Количество").style("font-size", "15px").attr("alignment-baseline", "middle")
